...
func TestValidateStruct(t *testing.T) {
	...
	tests := []struct {
		...
		// normal rules
		{
			"t2.1",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateAbc{}), Field(&m1.B, &validateXyz{})},
			"",
		},
		{
			"t2.2",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateXyz{}), Field(&m1.B, &validateAbc{})},
			"A: error xyz; B: error abc.",
		},
		{
			"t2.3",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateXyz{}), Field(&m1.c, &validateXyz{})},
		...
		{"t2.5", &m1, []*FieldRules{Field(&m1.F, Length(0, 5))}, ""},
		{
			"t2.6",
			&m1,
			[]*FieldRules{Field(&m1.H, Each(&validateAbc{})), Field(&m1.I, Each(&validateAbc{}))},
			"",
		},
		{
			"t2.7",
		...
		{"t7.6", &m2, []*FieldRules{Field(&m2.F, Skip.When(true), NotNil)}, ""},
		{
			"t7.7",
			&m2,
			[]*FieldRules{Field(&m2.F, Skip.When(false), Required)},
			"F: cannot be blank.",
		},
		...
		{"t8.6", &m4, []*FieldRules{Field(&m4.Model3)}, ""},
		{
			"t8.7",
			&m3,
			[]*FieldRules{Field(&m3.A, Required), Field(&m3.B, Required)},
			"A: cannot be blank; B: cannot be blank.",
		},
		{
			"t8.8",
			&m3,
			[]*FieldRules{Field(&m4.A, Required)},
		...
		// internal error
		{
			...
			&m5,
			[]*FieldRules{
				Field(&m5.A, &validateAbc{}),
				Field(&m5.B, Required),
				Field(&m5.A, &validateInternalError{}),
			},
			"error internal",
		},
	}
...
func TestValidateStructWithContext(t *testing.T) {
	...
	tests := []struct {
		...
		// normal rules
		{
			"t1.1",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateContextAbc{}), Field(&m1.B, &validateContextXyz{})},
			"",
		},
		{
			"t1.2",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateContextXyz{}), Field(&m1.B, &validateContextAbc{})},
			"A: error xyz; B: error abc.",
		},
		{
			"t1.3",
			&m1,
			[]*FieldRules{Field(&m1.A, &validateContextXyz{}), Field(&m1.c, &validateContextXyz{})},
			"A: error xyz; c: error xyz.",
		},
		...
		// internal error
		{
			...
			&m2,
			[]*FieldRules{
				Field(&m2.A, &validateContextAbc{}),
				Field(&m2.B, Required),
				Field(&m2.A, &validateInternalError{}),
			},
			"error internal",
		},
	}
